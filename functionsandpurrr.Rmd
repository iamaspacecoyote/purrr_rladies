...
I want to fit a logistic regression to understand if any changes occur over the decades. First, I create a time factor variable and a Bechdel test variable.

<div class="fold o">
```{r map5, out.width="75%", fig.align='left'}
bechdel_mod <- bechdel %>%
  dplyr::mutate(time_factor = dplyr::case_when(
                      year %in% 1970:1979 ~ '1970s',
                      year %in% 1980:1989 ~ '1980s',
                      year %in% 1990:1999 ~ '1990s',
                      year %in% 2000:2009 ~ '2000s',
                      year %in% 2010:2019 ~ '2010s'),
                bechdel_test = dplyr::case_when(binary == 'PASS' ~ 0,
                                                binary == 'FAIL' ~ 1))
```
</div>
...
Now I fit a logistic regression using the budget and domestic gross of the movie to predict whether or not it will pass the Bechdel test.
<div class="fold o">
```{r map7}
modeled_bech <- by_time %>% 
  #Fit logistic regression
  purrr::map(~ glm(bechdel_test ~ budget_2013 + domgross, 
...
Now I plot the coefficient results using the `ggplot` package.
...
Say I want to summarize the budget by whether or not they passed the Bechdel test for movies in each decade.
...
all.equal(bech_1990, bech_yr('1990s'))
```
...
Hints: use `lm` instead of `glm`; use log(domgross + 1) as dependent variable
<div class="fold s o">
```{r maplm}
modeled_bech_lm <- by_time %>% 
  #Fit logistic regression
  purrr::map(~ lm(log(domgross+1) ~ bechdel_test + budget_2013, data = .x)) %>% 
  #Using broom::tidy to make the output nicer for processsing
  purrr::map(~ broom::tidy(.x)) %>%
  #Grabbing variable names and point estimates
  purrr::map(~ dplyr::select(.x, term, estimate, std.error)) %>%
